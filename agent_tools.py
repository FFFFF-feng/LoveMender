...
# MCP 工具附加说明（当 MCP 工具可用时，追加到提示词后面）
# 这里只给一个总体说明，具体工具的描述 LangChain 会自动注入
MCP_TOOL_GUIDANCE = """

【GitHub 开发工具说明】
你还可以使用 GitHub 相关工具来帮助用户解决代码和开发问题：
- 搜索仓库、查看代码文件、了解项目结构
- 查看 Issue、PR、提交记录等开发信息
- 当用户询问代码相关、GitHub 项目、开源仓库等问题时可以调用

注意：
- 只有在用户明确提到 GitHub、代码、仓库、开源项目等相关话题时才使用
- 情感咨询类问题不要调用 GitHub 工具
- 调用工具前先理解用户意图，选择最合适的工具
""".strip()
